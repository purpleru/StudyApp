		...
		$.ajax({
			...
			success: function(data) {
				var code = data['code'],
					message = data['msg'];
				if (code == 200) {
					msg.show().removeClass('alert-danger').find('span:first').removeClass(
						'glyphicon-exclamation-sign');
					msg.find('[data-msg="msg"]').html(message + ',一般10分钟左右即可刷取完成!');
				} else if (code == 201) {
					display('请重新登陆账号');
				} else {
					msg.show().addClass('alert-danger').find('span:first').addClass(
						'glyphicon-exclamation-sign');
...
function lemonInit(el) {
	...
	var $el = $(el),
		$place = $el.find('#place'),
		$school = $el.find('#schools'),
		schoolInfo = userInit.schoolInfo || {};
	...
	function getSchollLists($option) {
		...
		$.get('/lemonSchool/schoollist', {
			...
			if (data.code === 1000) {
				...
				localStorage.setItem('baseURL', $checkedOption.attr('data-baseURL'));
			}
		});
	}
	...
	$.get('/lemonSchool/portal', function(data) {
		...
		localStorage.setItem('urlHost', $checkedOption.attr('data-urlHost'));
